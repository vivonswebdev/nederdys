...
import { Tb, BilingualInstruction, BilingualText, Bi } from "@/components/ui/BilingualText";
import { UI } from "@/lib/bilingual";
import { sounds } from "@/lib/sounds";
import { FrChallenge } from "@/data/fr/types";

/** Lecture en français de Belgique (les jeux FR travaillent la langue française). */
function speakFr(text: string, rate = 0.85) {
...
interface Props {
  ...
  pool: FrChallenge[];
  sessionSize?: number;
  /** Rendu personnalisé de l'énoncé (par défaut : le champ `prompt`). */
  renderPrompt?: (challenge: FrChallenge) => ReactNode;
  optionsClassName?: string;
  /** Masque l'énoncé écrit (jeux d'écoute pure comme la Dictée Muette). */
  hidePrompt?: boolean;
}

/**
 * Squelette commun des jeux de français à choix multiple — pendant FR de
 * NlQuizGame / MathQuizGame : XP, difficulté adaptative et audio fr-BE.
 */
export function FrQuizGame({
  gameId,
  emoji,
  titleKey,
  instructionKey,
  ...
  const [session, setSession] = useState<FrChallenge[]>(() => build());
  ...
  useEffect(() => {
    if (!challenge) return;
    setOptions(shuffle(challenge.options));
    setFeedback(null);
    setSelected(null);
    const t = window.setTimeout(() => speakFr(challenge.speak), 400);
  ...
  const handleSelect = (option: string) => {
    ...
    setSelected(option);
    const ok = option === challenge.correctAnswer;
    if (ok) {
      sounds.correct();
      setFeedback("correct");
      setScore((s) => s + 1);
        ...
        <AnimatePresence mode="wait">
          {!done && challenge ? (
            ...
            >
              <Button
                variant="outline"
                onClick={() => speakFr(challenge.speak)}
              ...
              {!hidePrompt && (
                <div className="bg-card border border-border rounded-2xl p-6 mb-6 shadow-lg w-full text-center font-dyslexic">
                  {renderPrompt ? (
                    ...
                    <>
                      <p className="text-3xl md:text-4xl font-bold">{challenge.prompt}</p>
                      {challenge.hint && (
                        <p className="text-sm text-muted-foreground mt-2">{challenge.hint}</p>
                      )}
                    </>
                  )}
                </div>
              ...
              <div className={optionsClassName}>
                {options.map((option) => {
                  ...
                  const good = feedback && option === challenge.correctAnswer;
                  return (
                    <button
                      key={option}
